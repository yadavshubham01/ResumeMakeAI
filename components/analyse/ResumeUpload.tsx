...
interface ResumeUploadProps {
  selectedJob: any | null; // replace `any` with proper type if you know it
  setSelectedJob: Dispatch<SetStateAction<any | null>>;
  setParsedData: Dispatch<SetStateAction<any | null>>;
  setPdfUrl: Dispatch<SetStateAction<string>>;
  setAnalysis: Dispatch<SetStateAction<any | null>>;
}
...
const ResumeUpload: React.FC<ResumeUploadProps> = ({
 ...
 const convertToImage = async (
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber++) {
    ...
    if (!ctx) continue; // safety check

    // ✅ Add `canvas` property as required by the types
    await page.render({
      canvasContext: ctx,
      viewport: viewport,
      canvas: canvas, 
